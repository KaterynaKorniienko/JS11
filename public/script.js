...
var update = injectStylesIntoStyleTag_default()((BrightTheme_default()), options);
...
// EXTERNAL MODULE: ./node_modules/mini-css-extract-plugin/dist/loader.js??ruleSet[1].rules[1].use[1]!./node_modules/css-loader/dist/cjs.js!./src/app.css
var app = __webpack_require__(970);
...
var app_update = injectStylesIntoStyleTag_default()((app_default()), app_options);




       /* harmony default export */ const src_app = ((app_default()) && (app_default()).locals ? (app_default()).locals : undefined);
...
var CountryInfo = /*#__PURE__*/function () {
  function CountryInfo(uri, refs) {
    var _this = this;

    _classCallCheck(this, CountryInfo);

    _defineProperty(this, "renderInfo", function (infoArr) {
      _this.refs.dataRef.innerHTML = "";
      var infoArrLength = infoArr.length;
      ...
      if (infoArrLength > 10) {
        ...
        _this.refs.dataRef.append(infoArrLength > 2 ? _this.createList(infoArr) : _this.createTable(infoArr[0]));
      }
    });

    _defineProperty(this, "fetchCountryInfo", function (inputValue) {
      fetch(_this.uri + inputValue).then(function (res) {
        if (!res.ok) {
          (0,PNotify.alert)({
            title: "Status: ".concat(res.status),
            text: "Country ".concat(res.statusText),
            type: "notice"
          });
        }
    ...
    _defineProperty(this, "onInputCountryName", function (event) {
      if (event.target.value) {
        _this.fetchCountryInfo(event.target.value);
      }
    });

    _defineProperty(this, "onRefClick", function (event) {
      ...
      _this.fetchCountryInfo(event.target.innerHTML);
    });
  ...
  _createClass(CountryInfo, [{
    ...
    value: function createTable(data) {
      ...
      row1.innerHTML += "<td><b>".concat(data.name.common, "</b></td>");
      tableBody.append(row1);
      var row1a = document.createElement("tr");
      row1a.innerHTML += "<td>Official name:</td>";
      row1a.innerHTML += "<td>".concat(data.name.official, "</td>");
      tableBody.append(row1a);
      var row2 = document.createElement("tr");
      row2.innerHTML += "<td>Capital:</td>";
      row2.innerHTML += "<td>".concat(data.capital[0], "</td>");
      tableBody.append(row2);
      var row3 = document.createElement("tr");
      row3.innerHTML += "<td>Population:</td>";
      row3.innerHTML += "<td>".concat(data.population, "</td>");
      tableBody.append(row3);
      var row4 = document.createElement("tr");
      row4.innerHTML += "<td>Languages:</td>";
      var langArray = Object.values(data.languages);

      if (langArray.length === 1) {
        row4.innerHTML += "<td>".concat(langArray[0], "</td>");
      } else {
        var str = langArray.map(function (language) {
          return language;
        });
        row4.innerHTML += "<td>".concat(str, "</td>");
      }

      tableBody.append(row4);
      var row5 = document.createElement("tr");
      row5.innerHTML += "<td>Flag:</td>";
      row5.innerHTML += "<td><img src=".concat(data.flags.png, " alt=\"Flag image\"</td>");
      tableBody.append(row5);
      var row6 = document.createElement("tr");
      row6.innerHTML += "<td>Coat of Arms:</td>";
      row6.innerHTML += "<td><img src=".concat(data.coatOfArms.png, " alt=\"Flag image\"</td>");
    ...
    key: "createList",
    value: function createList(dataArr) {
      var ul = document.createElement("ul");
      var dataItems = dataArr.map(function (_ref) {
        var name = _ref.name;
      ...
      ul.append.apply(ul, _toConsumableArray(dataItems));
      return ul;
    } // название, столица, население, языки и флаг.

  }, {
    key: "loadListeners",
    value: function loadListeners() {
      this.refs.inputSearchByName.addEventListener("input", _.debounce(this.onInputCountryName, 500));
